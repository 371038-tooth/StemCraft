...
| ピッチ・テンポ変更 | キーとBPMを自動検出し、高品質かつ劇的に高速なピッチ（音の高さ）やテンポ（速度）の調整機能 |
| 再生コントロール | 再生・一時停止・停止・シークバーによる位置移動 |
| 分離音の書き出し | 編集・分離した音源を WAV, MP3, FLAC, OGG 形式で保存 |
| 直感的なUI | モダンダークなデザインテーマと、AI分離および変換処理のリアルタイムな進捗表示 |
...
| ライブラリのインストール | PyQt5・librosa・pedalboard 等の必要パッケージを取得 | 3〜5 分 |
